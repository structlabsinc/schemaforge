...
## [2.1.0] - 2026-01-14
### Added
- **Native CLUSTERED Index Support**: MSSQL indexes now properly parse and generate `CLUSTERED`/`NONCLUSTERED` keywords.
- **Constraint-Aware Migrations**: Generators automatically emit `DROP CONSTRAINT` before modifying Primary Key columns.
- **`is_clustered` Field**: Added to `Index` model for accurate drift detection.

### Fixed
- **PARTITION BY Double-Prefix Bug**: Fixed Postgres/MySQL generators from emitting `PARTITION BY PARTITION BY`.
